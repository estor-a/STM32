...
static uint32_t SolvePLL(PLL_Clocks_t* PLL, uint8_t Skip, uint8_t SweepAll);
...
static uint32_t IsSameOrBetterCoef(PLL_Node_t* X) {
  ...
  uint32_t ErrorHz = Absolute_Difference(X->TargetHz, X->Hz);
...
static uint32_t StartNextPossiblePLLCoef(PLL_Node_t* T) {
...
static uint32_t GetNextPossiblePLLCoef(PLL_Node_t* T) { // step = +1, +2 if numerator, -1 -2 if denominator
...
static uint32_t IsNotIsDoneYetNextPossiblePLLCoef(PLL_Node_t* T) {
...
static uint32_t SolvePLL(PLL_Clocks_t* PLL, uint8_t Skip, uint8_t SweepAll) {
  ...
  uint32_t NbOfSolutions = 0;
  uint32_t FoundOneSolution = FALSE;
...
#if 0 // Disabled as this is dynamic clock tree change
// Now we need to identify the gearing
// APB1: RCC_HCLK_Div1
// APB2: (same)<<3
// HPRE: RCC_SYSCLK_Div1
const uint32_t Gears[5][5] = { 
...
void SetMCU_Gear(MCU_Clocks_t* C, uint32_t Gear) { // Gear 1..5 MaxGear 1..5
...
#if 0 // This is to solve the full clock tree with solution. 
...
void SetMCU_Gears(MCU_Clocks_t* C) {
  ...
  uint32_t ratio = C->OutSysClk_Hz.Value / C->OutSysClk_Hz.Min;
...
#ifndef _SEB_MATH_H_
static int32_t Interpolate_s32 (int32_t x0, int32_t x1, int32_t y0, int32_t y1, int32_t x)
{ 
// code for 32 bit mcu
    int32_t dwQ;
...
void EstimatePower(MCU_Clocks_t* C) {
  ...
  uint32_t Hz = C->OutSysClk_Hz.Value;
  uint32_t mA,mW;
  if(Hz==0) while(1); // error, can't be clock stopped
  
  for(uint32_t n=1;n<countof(HzTo_mA); n++) {
