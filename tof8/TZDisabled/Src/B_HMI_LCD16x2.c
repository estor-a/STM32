...
IO_Pin_t LCD_RST_STMod = {  GPIOA,          {    GPIO_PIN_8, GPIO_MODE_OUTPUT_PP, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t LCD_RS_STMod = {  GPIOA,          {    GPIO_PIN_7, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t LCD_CS_STMod = {  GPIOE,          {    GPIO_PIN_12, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t LCD_SCL_STMod = {  GPIOE,          {    GPIO_PIN_13, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t LCD_SI_STMod = {  GPIOE,          {    GPIO_PIN_15, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 0,   } };
...
IO_Pin_t LCD_RSToff_STMod = {  GPIOA,          {    GPIO_PIN_8, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t LCD_RSoff_STMod = {  GPIOA,          {    GPIO_PIN_7, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t LCD_CSoff_STMod = {  GPIOE,          {    GPIO_PIN_12, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t LCD_SCLoff_STMod = {  GPIOE,          {    GPIO_PIN_13, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t LCD_SIoff_STMod = {  GPIOE,          {    GPIO_PIN_15, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };

// Joystick related
IO_Pin_t JOY_UP_STMod = {  GPIOA,          {    GPIO_PIN_1, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t JOY_DOWN_STMod = {  GPIOA,          {    GPIO_PIN_4, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t JOY_LEFT_STMod = {  GPIOA,          {    GPIO_PIN_3, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t JOY_RIGHT_STMod = {  GPIOA,          {    GPIO_PIN_2, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
IO_Pin_t JOY_SEL_STMod = {  GPIOA,          {    GPIO_PIN_0, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_LOW, 0,   } };
...
int32_t HL_IsPlugged(void) {
  ...
  // Let's initial a message
   LCD_16x2_ShowTemporaryMessage_0(&pADD->LCD_16x2, 1000, pADD->pADD->StringID );
...
//  uint16_t Lines = 1;
  ...
  case 4: LCD_16x2_Printf(pL, "MEMs: \nACC x=%D g", Motion3D.gravity_x100 ); break;
  case 5: LCD_16x2_Printf(pL, "ACC %D g\nr=%D p=%D", Motion3D.gravity_x100, Motion3D.roll_deg, Motion3D.pitch_deg ); break;
  case 6: LCD_16x2_Printf(pL, "r=%D p=%D\nMAG %D uT", Motion3D.roll_deg, Motion3D.pitch_deg, Magneto3D.uT); break;
  case 7: LCD_16x2_Printf(pL, "MAG %D uT\nr=%D p=%D", Magneto3D.uT,Magneto3D.roll_deg,Magneto3D.pitch_deg); break;
  case 8: LCD_16x2_Printf(pL, "r=%D p=%D\nGYR x=%D", Magneto3D.roll_deg, Magneto3D.pitch_deg,0); break;
...
uint8_t GUI_Manage_AddonMenu(HM_AddOn_t* pADD) {
  ...
  if(JOY_SelectPressed(pJOY)&&(pADD->pGUI->IndexVertical==1)) {
      if(IO_PinGet(pL->BL)==1) IO_PinSetLow(pL->BL); else IO_PinSetHigh(pL->BL);
      pADD->pGUI->LCD_UpdateRequested = 1;
  }
...
uint8_t GUI_Render_AddonMenu(HM_AddOn_t* pADD) {
  ...
  case 1: ((IO_PinGet(pL->BL)==1))?LCD_16x2_Printf(pL, "Backlight [ON]\n" ):LCD_16x2_Printf(pL, "Backlight [OFF]\n" ); if(--Lines) break;
...
uint8_t GUI_Manage_SPIpMenu(HM_AddOn_t* pADD) {
  ...
  if(JOY_LeftPressed(pJOY)) { // go back to main top menu
    ...
    case 0: pADD->pGUI = &pADD->GUIs[0]; // go back to main page
            return 0; // finished
    case 2: GUI_Slave = (GUI_Slave + MAX_SLAVE - 1) % MAX_SLAVE; 
            return 0;
  ...
  if(JOY_RightPressed(pJOY)) { // go back to main top menu
    pADD->pGUI->LCD_UpdateRequested = 1;
    switch(pADD->pGUI->IndexVertical) {
    case 2: GUI_Slave = (GUI_Slave + 1) % MAX_SLAVE; 
            return 0;
...
//  uint16_t Lines = 1;
  ...
  switch(pADD->pGUI->IndexVertical) {
  case 0: LCD_16x2_Printf(pL, "SPI+ Menu %d/%d\nEdit SIF Xbar", pSPIP->NbOfSlaves, MAX_SLAVE );     break;
  case 1: LCD_16x2_Printf(pL, "Edit SIF Xbar S\nSlave %d/%d", GUI_Slave, MAX_SLAVE );       break;
...
ByteVein_t* pHL_Report = &BV_to_SPIP[STM_SERIAL];
