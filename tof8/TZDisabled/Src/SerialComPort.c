...
//  ErrorStatus Error;
  ...
  uint32_t CMD;
  CMD = ((9600L)<<12) | 1; // default SPIP going to SWD port (you have to setup the serial interface on PC side for 115200bps like BT dongle
  ...
  BTEL_Disable();// Disable the Android Protocol BTEL_Enable(); // Activate the bluetooth electronics protocol.
