...
void BTEL_Incoming(void) { // any chars coming from the phone's app?
  ...
  while(u8fifo_from_RX[BT_SERIAL].bCount) { // some characters to process
    
    // read a char from incoming FIFO
    c = (uint8_t) Clipu8fifo_Down(&u8fifo_from_RX[BT_SERIAL]);
...
void BTEL_TestPrintf(void) {
  ...
  while(1) {
    ...
    BTEL_Printf(&u8fifo_to_TX[BT_SERIAL], "*TBriskV2*U%i*V%D*W%C*%M*", v1, v1, v1, v1);
    HAL_Delay(1000);
    v1++;    
    BTEL_Printf(&u8fifo_to_TX[BT_SERIAL], "*TAdd-on*U%i*V%D*W%C*%M*", v2, v2, v2, v2);
    HAL_Delay(1000);
    v1++;    
  }
